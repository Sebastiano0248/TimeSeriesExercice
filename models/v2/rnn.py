"""
LSTM-based RNN model for univariate time series forecasting.
Follows the standard model interface required by forecasting_pipeline.ipynb.

Uncertainty quantification via MC Dropout:
  During predict(), dropout stays active and we run MC_SAMPLES forward passes.
  The spread across samples gives the predictive uncertainty.

Prediction strategy: recursive (autoregressive) — predict 1 step, feed it
back as input, repeat h times. Uncertainty accumulates naturally over horizon.
...
MC_SAMPLES  = 300   # forward passes for MC Dropout uncertainty
# ──────────────────────────────────────────────────────────────────────────────
...
class _LSTMNet(nn.Module):
    ...
    def forward(self, x):
        # x: (batch, seq_len, 1)
        out, _ = self.lstm(x)
        out = self.drop(out[:, -1, :])   # only last timestep
        return self.fc(out).squeeze(-1)


class RNNModel:
    def __init__(self, net, mean_, std_, last_seq_norm):
        self.net          = net
        self.mean_        = float(mean_)
        self.std_         = float(std_)
        self._last_seq    = last_seq_norm   # normalized, shape (SEQ_LEN,)

    def predict(self, h: int) -> dict:
        # MC Dropout: keep dropout active (net.train()) during inference
        self.net.train()
...
def train(y_train, y_val=None) -> RNNModel:
    values = y_train.values.astype(np.float32)

    mean_ = values.mean()
    std_  = values.std() + 1e-8     # avoid division by zero
    ...
    # Build val loader if y_val provided (used only for early stopping)
    val_loader = None
    if y_val is not None:
        norm_val = (y_val.values.astype(np.float32) - mean_) / std_
        # prepend last SEQ_LEN train points so the first val window is valid
    ...
    return RNNModel(net, mean_, std_, last_seq_norm=norm[-SEQ_LEN:])
